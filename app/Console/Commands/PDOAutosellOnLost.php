...
class PDOAutosellOnLost extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cryptobot:pdo_autosell_on_lost {max_date}';
    ...
    */
    ...
    public function handle()
    {
        ...
        if(true)
        {
            ...
            $query = 'SELECT * FROM wallets WHERE available > 0';
            $req = $pdo->prepare($query);
            $req->execute();

            $wallets = $req->fetchAll();
            ...
            foreach ($wallets as $wallet)
            {
                ...
                $req->execute([
                    ...
                    'buy_strategy_name' => $this->buy_strategy_name,
                ]);

                $last_transaction = $req->fetch();
                ...
                $query = "  SELECT * FROM candles_1m
                            WHERE created_at > :last_transaction_date
